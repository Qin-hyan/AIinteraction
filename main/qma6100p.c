...
#define QMA6100P_INT_STATUS           0x09u   /* interrupt / data-ready status */
#define QMA6100P_STATUS               0x0Au   /* chip status */
#define QMA6100P_FIFO_FRAME_CTR       0x0Eu
#define QMA6100P_RANGE                0x0Fu   /* accelerometer range (was ACCEL_CONFIG) */
#define QMA6100P_ODR_BW              0x10u   /* ODR / bandwidth (was PWR_MGMT_1) */
#define QMA6100P_PM                  0x11u   /* Power Management – bit7 = MODE */
#define QMA6100P_INTR_ENABLE          0x16u
#define QMA6100P_INTR_FIFO_EN         0x17u
#define QMA6100P_INTR_MOT_EN          0x18u

/* interrupt map 0x19–0x1C */
...
#define QMA6100P_INT_FIFO_ST          0x0Bu   /* FIFO interrupt status */
#define QMA6100P_NVM_LOAD             0x33u
#define QMA6100P_SOFT_RESET           0x36u   /* soft-reset trigger */
#define QMA6100P_FIFO_MODE            0x3Eu
#define QMA6100P_FIFO_DATA            0x3Fu
#define QMA6100P_MCLK                 0x4Au   /* MCLK / clock config */
#define QMA6100P_REG_56               0x56u   /* internal config */
#define QMA6100P_REG_5F               0x5Fu   /* internal config */
...
/* ---- [DIAG] public register read wrapper ---- */
esp_err_t qma6100p_read_reg(qma6100p_handle_t sensor, const uint8_t reg_addr,
                             uint8_t *data, const uint8_t len)
{
    return qma6100p_read(sensor, reg_addr, data, len);
}
...
esp_err_t qma6100p_wake_up(qma6100p_handle_t sensor)
{
    ...
    /* ============================================================
     * QMA6100P Official Initialization Sequence
     * ============================================================ */

    /* Step 1: Soft Reset */
    tmp = 0xB6;
    ret = qma6100p_write(sensor, QMA6100P_SOFT_RESET, tmp);
    ESP_RETURN_ON_ERROR(ret, TAG, "soft-reset write 0xB6 failed");
    vTaskDelay(pdMS_TO_TICKS(5));

    tmp = 0x00;
    ret = qma6100p_write(sensor, QMA6100P_SOFT_RESET, tmp);
    ESP_RETURN_ON_ERROR(ret, TAG, "soft-reset write 0x00 failed");
    vTaskDelay(pdMS_TO_TICKS(10));

    /* Step 2: Enter Active mode */
    ret = qma6100p_write(sensor, QMA6100P_PM, 0x80);
    ESP_RETURN_ON_ERROR(ret, TAG, "PM write 0x80 failed");
    ret = qma6100p_write(sensor, QMA6100P_PM, 0x84);
    ESP_RETURN_ON_ERROR(ret, TAG, "PM write 0x84 failed");

    /* Step 3: Vendor initialization */
    ret = qma6100p_write(sensor, QMA6100P_MCLK, 0x20);
    ESP_RETURN_ON_ERROR(ret, TAG, "MCLK (0x4A) write failed");

    ret = qma6100p_write(sensor, QMA6100P_REG_56, 0x01);
    ESP_RETURN_ON_ERROR(ret, TAG, "REG_56 write failed");

    ret = qma6100p_write(sensor, QMA6100P_REG_5F, 0x80);
    ESP_RETURN_ON_ERROR(ret, TAG, "REG_5F write 0x80 failed");
    vTaskDelay(pdMS_TO_TICKS(2));

    ret = qma6100p_write(sensor, QMA6100P_REG_5F, 0x00);
    ESP_RETURN_ON_ERROR(ret, TAG, "REG_5F write 0x00 failed");
    vTaskDelay(pdMS_TO_TICKS(2));

    /* Step 4: Range = 8G, ODR/BW = 100Hz / 250Hz */
    ret = qma6100p_read(sensor, QMA6100P_RANGE, &tmp, 1);
    ESP_RETURN_ON_ERROR(ret, TAG, "read RANGE failed");
    tmp = (tmp & 0xF0) | (ACCE_FS_8G & 0x0F);
    ret = qma6100p_write(sensor, QMA6100P_RANGE, tmp);
    ESP_RETURN_ON_ERROR(ret, TAG, "write RANGE (8G) failed");

    /* ODR/BW: BW=250Hz, ODR=100Hz */
    ret = qma6100p_write(sensor, QMA6100P_ODR_BW, 0x29);
    ESP_RETURN_ON_ERROR(ret, TAG, "write ODR_BW failed");

    /* Step 5: Final PM = 0x84 (MCLK=51.2kHz + Active) */
    ret = qma6100p_write(sensor, QMA6100P_PM, 0x84);
    ESP_RETURN_ON_ERROR(ret, TAG, "final PM write 0x84 failed");

    /* Verify MODE = Active */
    ret = qma6100p_read(sensor, QMA6100P_PM, &tmp, 1);
    ESP_RETURN_ON_ERROR(ret, TAG, "verify PM read failed");
    if (!(tmp & 0x80)) {
        ESP_LOGE(TAG, "MODE=STANDBY after init! PM=0x%02X", tmp);
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "Init OK — MODE=ACTIVE PM=0x%02X", tmp);

    /* Step 6: FIFO bypass — route data to 0x01-0x06 output registers */
    ret = qma6100p_write(sensor, QMA6100P_FIFO_MODE, 0x00);
    ESP_RETURN_ON_ERROR(ret, TAG, "FIFO bypass write failed");
...
esp_err_t qma6100p_config(qma6100p_handle_t sensor, const qma6100p_acce_fs_t acce_fs)
{
    uint8_t config_reg;
    esp_err_t ret;
    ret = qma6100p_read(sensor, QMA6100P_RANGE, &config_reg, 1);
    if (ESP_OK != ret) {
        return ret;
    }
    config_reg = (config_reg & 0xf0) | (acce_fs & 0x0f);
    return qma6100p_write(sensor, QMA6100P_RANGE, config_reg);
}
...
esp_err_t qma6100p_get_acce_sensitivity(qma6100p_handle_t sensor, float *const acce_sensitivity)
{
    esp_err_t ret;
    uint8_t acce_fs;
    ret = qma6100p_read(sensor, QMA6100P_RANGE, &acce_fs, 1);
    acce_fs &= 0b1111;
    switch (acce_fs) {
    case ACCE_FS_2G:
...
esp_err_t qma6100p_get_interrupt_status(qma6100p_handle_t sensor, uint8_t *const out_intr_status)
{
    ...
    ret = qma6100p_read(sensor, QMA6100P_INT_FIFO_ST, out_intr_status, 1);
